...
def mainRun():
    ...
    if args.subparserID == 'cffq':
        ...
        fr = msbwt.findIndicesOfStr(args.kmer.encode())
        rv = msbwt.findIndicesOfStr(MultiStringBWT.reverseComplement(args.kmer).encode())
        print("Forward: %s, Reverse: %d, Total: %d" %(fr[1]-fr[0], rv[1]-rv[0], fr[1]-fr[0] + rv[1]-rv[0]))
        
        #dump the seqs if request
        if args.dumpSeqs:
            for x in range(fr[0], fr[1]):
                dInd = msbwt.getSequenceDollarID(x)
                print ("Forward read:", msbwt.recoverString(dInd)[1:].decode()+','+str(dInd))
            for x in range(rv[0], rv[1]):
                dInd = msbwt.getSequenceDollarID(x)
                print ("Reverse read:", msbwt.recoverString(dInd)[1:].decode()+','+str(dInd))
        ...
        for line in open(args.kmerFile, 'r'):
            kmer = line.strip('\n')
            c = msbwt.countOccurrencesOfSeq(kmer.encode())
            if args.reverseComplement:
                rc = msbwt.countOccurrencesOfSeq(MultiStringBWT.reverseComplement(kmer).encode())
        ...
        print ("Not a valid command: use msbwt3 -h for help")
